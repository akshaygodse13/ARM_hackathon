...
; r is started from 1 to 2400 value as it will take 8 loops 2400/360=8 
		VLDR.F32 s22,=1.0				;radius starting value
		VMOV.F32 R0,S22
		VLDR.F32 S26,=3000.0				
		VMOV.F32 R1,S26
		VLDR.F32 S27,=3000.0				
...
__DRAWSPIRAL FUNCTION						
		...
		VLDR.F32 S25,=2400.0  				;max radius value
		VLDR.F32 S24,=1						;radius increment
		VLDR.F32 S9,=0.0 					;This isthe input for sine and cosine
CORDC	VMOV.F32 R0,S9
		;BL printMsg		
		;BL printComma
		BL __SINE							
		VMUL.F32 S5,S5,S22					
		...
		BL printMsg
		BL printNextline
		VADD.F32 S9,S9,S20					;angle increement by 1
		VADD.F32 S22,S22,S24                 ;radius increment by 0.5
		VCMP.F32 S9,S21						;for initializing angle to 0
		VMRS APSR_nzcv, FPSCR
		IT EQ;
		VMOVEQ.f32 S9,S20;
		VCMP.F32 S22,S25						;max Radius reaches 2400
		VMRS APSR_nzcv, FPSCR
		BNE CORDC		
...
__SINE FUNCTION			;Here input is S9 and output is S5 = sin()
        VLDR.F32 S1,=90.0
        VMOV.F32 S0,S9
        VCMP.F32 S9,S1
		VMRS APSR_nzcv, FPSCR
        ...
        BLT SINPI
		VLDR.F32 S1,=360.0
        VSUB.F32 S0,S9,S1   				;Stores angle range from -PI/2 to PI/2
...
MULSIN	VMUL.F32 S3,S3,S2					;S3*(-x^2)
		...
		SUB R1,R1,#1						
		CMP R1,#0
		BNE MULSIN							;Branch to next term calculation
